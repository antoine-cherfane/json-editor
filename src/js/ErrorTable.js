...
export class ErrorTable {
  ...
  setErrors (errors, errorLocations) {
    ...
    if (this.errorTableVisible && errors.length > 0) {
      ...
      errors.forEach((error) => {
        ...
        if (!isNaN(error.line)) {
          ...
          const errLoc = errorLocations.find(
            (loc) => loc.path === error.dataPath
          )
          if (errLoc) {
            line = errLoc.line + 1
          }
        ...
        if (typeof error === 'string') {
          ...
          const pre = document.createElement('pre')
          pre.appendChild(
            document.createTextNode(error.message.replace(/<br>/gi, '\n'))
          )
          td4.appendChild(pre)
    ...
    if (validationErrorsCount > 0) {
      ...
      this.dom.validationErrorIcon.onclick =
        this.toggleTableVisibility.bind(this)
    } else {
    ...
    if (hasParseErrors) {
      ...
      this.dom.parseErrorIndication.onclick =
        this.toggleTableVisibility.bind(this)
    } else {
      this.dom.parseErrorIndication.style.display = 'none'
    }
