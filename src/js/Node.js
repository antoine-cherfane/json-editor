...
export class Node {
  ...
  findNode (jsonPath) {
    ...
    while (node && path.length > 0) {
      ...
      if (typeof prop === 'number') {
        if (node.type !== 'array') {
          throw new Error(
            'Cannot get child node at index ' + prop + ': node is no array'
          )
        }
  ...
  _updateDomField () {
    ...
    if (domField) {
      const tooltip = makeFieldTooltip(
        this.schema,
  ...
  onKeyDown (event) {
    ...
    if (keynum === 13) {
      ...
      if (altKey && !shiftKey) {
        ...
        if (nextDom) {
          ...
          if (
            ...
            oldNextNode =
              oldParent.childs[lastNode.getIndex() + 1] || oldParent.append
      ...
      if (altKey && !shiftKey) {
        ...
        const nextNode2 =
          nextNode && (nextNode._nextNode() || nextNode.parent.append)
  ...
  _showColorPicker () {
    if (
      ...
      const colorAnchor = createAbsoluteAnchor(
        this.dom.color,
        this.editor.getPopupAnchor()
      )
   ...
   */
  showTransformModal () {
    const { modalAnchor, createQuery, executeQuery, queryDescription } =
      this.editor.options
    const json = this.getValue()
...
Node.onDragStart = (nodes, event) => {
  ...
  const offsetY =
    getAbsoluteTop(draggedNode.dom.tr) - getAbsoluteTop(firstNode.dom.tr)
...
Node.onDragEnd = (nodes, event) => {
  ...
  const newIndexRedo =
    sameParent && oldIndexRedo < newIndex ? newIndex + nodes.length : newIndex
