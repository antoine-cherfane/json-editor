...
import { isArray, isObject, uniqueMergeArrays, asyncExec } from './util'
...
export class SchemaTextCompleter {
  ...
  _buildSuggestions () {
    ...
    for (const refName in this.schemaRefs) {
      ...
      this._handleSchemaEntry(
        '',
        this.schemaRefs[refName],
        this.suggestionsRefs[refName]
      )
    }
  }
  ...
  _handleRef (currectPath, refName, suggestionsObj) {
    ...
    suggestionsObj[currectPath].refs = uniqueMergeArrays(
      suggestionsObj[currectPath].refs,
      [refName]
    )
  }
  ...
  _handleSchemaEntry (currectPath, schemaNode, suggestionsObj) {
    if (!schemaNode) {
      console.error(
        'SchemaTextCompleter: schema node is missing for path',
        currectPath
      )
      return
    }
    ...
    if (ofConditionEntry) {
      this._handleOfCondition(
        currectPath,
        schemaNode[ofConditionEntry],
        suggestionsObj
      )
      return
    }
  ...
  _handleObject (currectPath, schemaNode, suggestionsObj) {
    if (isObject(schemaNode.properties)) {
      ...
      suggestionsObj[currectPath].props =
        suggestionsObj[currectPath].props || []
      suggestionsObj[currectPath].props = uniqueMergeArrays(
        suggestionsObj[currectPath].props,
        props
      )
      props.forEach((prop) => {
  ...
  _handlePrimitive (currectPath, schemaNode, suggestionsObj) {
    ...
    if (isArray(schemaNode.examples)) {
      suggestionsObj[currectPath].examples =
        suggestionsObj[currectPath].examples || []
      suggestionsObj[currectPath].examples = uniqueMergeArrays(
        suggestionsObj[currectPath].examples,
        schemaNode.examples
      )
    }
    if (isArray(schemaNode.enum)) {
      ...
      suggestionsObj[currectPath].enum = uniqueMergeArrays(
        suggestionsObj[currectPath].enum,
        schemaNode.enum
      )
    }
  }
      ...
      Object.keys(pointers).forEach((ptr) => {
        asyncExec(() => {
          const matchPointersToPath = (pointer, currentSuggestions, path) => {
            const option = Object.keys(currentSuggestions).reduce(
              (last, key) => {
              ...
              if (currentSuggestions[option]?.refs?.length) {
                ...
                for (const idx in currentSuggestions[option].refs) {
                  ...
                  if (this.suggestionsRefs[refName]) {
                    const refSuggestion = matchPointersToPath(
                      pointer,
                      this.suggestionsRefs[refName],
                      `${path}${option}`
                    )
                    if (refSuggestion?.enum) {
                      mergedSuggestions.enum = uniqueMergeArrays(
                        mergedSuggestions.enum,
                        refSuggestion.enum
                      )
                    }
                    if (refSuggestion?.examples) {
                      mergedSuggestions.examples = uniqueMergeArrays(
                        mergedSuggestions.examples,
                        refSuggestion.examples
                      )
                    }
                    if (refSuggestion?.bool) {
                      mergedSuggestions.bool = uniqueMergeArrays(
                        mergedSuggestions.bool,
                        refSuggestion.bool
                      )
                    }
                    if (refSuggestion?.props) {
                      mergedSuggestions.props = uniqueMergeArrays(
                        mergedSuggestions.props,
                        refSuggestion.props
                      )
                    }
                  }
          ...
          if (pointers[ptr].key?.line === pos.row) {
            if (
              pos.column >= pointers[ptr].key.column &&
              pos.column <= pointers[ptr].keyEnd.column
            ) {
              selectedPtr = ptr.slice(0, ptr.lastIndexOf('/'))
            }
          }
          if (
            pointers[ptr].value?.line === pos.row &&
            pointers[ptr].value?.line === pointers[ptr].valueEnd?.line
          ) {
            // multiline values are objects
            if (
              pos.column >= pointers[ptr].value.column &&
              pos.column <= pointers[ptr].valueEnd.column
            ) {
              selectedPtr = ptr
            }
          }
          if (selectedPtr) {
            const chosenCompletions = matchPointersToPath(
              selectedPtr,
              this.suggestions,
              ''
            )
