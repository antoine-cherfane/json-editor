...
textmode.create = function (container, options = {}) {
  ...
  options.showErrorTable =
    options.showErrorTable !== undefined
      ? options.showErrorTable
      : ['text', 'preview']
...
textmode._onSchemaChange = function (schema, schemaRefs) {
  ...
  if (this.options.allowSchemaSuggestions && schema) {
    this.aceEditor.setOption('enableBasicAutocompletion', [
      new SchemaTextCompleter(schema, schemaRefs)
    ])
...
textmode.validate = function () {
  ...
  try {
    ...
    return validateCustom(json, this.options.onValidate).then(
...
textmode.setTextSelection = function (startPos, endPos) {
  ...
  if (this.textarea) {
    const startIndex = getIndexForPosition(
      this.textarea,
      startPos.row,
      startPos.column
    )
    const endIndex = getIndexForPosition(
      this.textarea,
      endPos.row,
      endPos.column
    )
