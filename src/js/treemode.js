        ...
        if (Array.isArray(customValidationPathErrors)) {
          return customValidationPathErrors
            ...
            .map((error) => {
              ...
              try {
                node =
                  error && error.path ? root.findNodeByPath(error.path) : null
              } catch (err) {
            ...
            .filter(
              (entry) =>
                entry && entry.node && entry.error && entry.error.message
            )
        } else {
          return null
        }
...
treemode.setDomSelection = function (selection) {
  ...
  if (selection.paths) {
    // multi-select
    const me = this
    const nodes = selection.paths.map((path) =>
      me.node.findNodeByInternalPath(path)
    )
...
treemode._onEvent = function (event) {
  ...
  if (node && node.selected) {
    ...
    if (
      ...
      hasParentNode(event.target, this.content)
    ) {
      this.deselect()
...
treemode.getSelection = function () {
  ...
  if (this.multiselection.nodes && this.multiselection.nodes.length) {
    if (this.multiselection.nodes.length) {
      ...
      const selection2 =
        this.multiselection.nodes[this.multiselection.nodes.length - 1]
