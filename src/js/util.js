...
export function getInnerText (element, buffer) {
  ...
  if (element.nodeValue) {
    ...
    const trimmedValue = removeReturnsAndSurroundingWhitespace(
      element.nodeValue
    )
    if (trimmedValue !== '') {
...
export function parsePath (jsonPath) {
  ...
  function parseIndex (end) {
    ...
    if (jsonPath[i] !== end) {
      throw new Error(
        'Invalid JSON path: unexpected end, character ' + end + ' expected'
      )
    }
...
export function improveSchemaError (error) {
  ...
  if (error.keyword === 'additionalProperties') {
    error.message =
      'should NOT have additional property: ' + error.params.additionalProperty
  }
...
export function sortObjectKeys (object, direction) {
  const sign = direction === 'desc' ? -1 : 1
  const sortedFields = Object.keys(object).sort(
    (a, b) => sign * naturalSort(a, b)
  )
