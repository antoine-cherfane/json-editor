...
describe('Node', () => {
  describe('_findSchema', () => {
    ...
    it('should find schema inside an array item', () => {
      ...
      assert.strictEqual(
        Node._findSchema(schema, {}, ['job']),
        schema.properties.job
      )

      assert.strictEqual(
        Node._findSchema(schema, {}, ['job', 0]),
        schema.properties.job.items
      )

      assert.strictEqual(
        Node._findSchema(schema, {}, ['job', 0, 'company']),
    ...
    describe('with $ref', () => {
      it('should find a referenced schema', () => {
        ...
        const path = ['foo']
        assert.strictEqual(
          Node._findSchema(schema, { foo: fooSchema }, path),
          fooSchema
        )
      })
    ...
    describe('with $ref to external definition', () => {
      it('should find a referenced schema', () => {
        ...
        assert.notStrictEqual(
          Node._findSchema(schema, { 'definitions.json': definitions }, path),
          foundSchema
        )
      })
    })
    describe('with pattern properties', () => {
      ...
      it('should find schema for multi-level pattern properties', () => {
        ...
        assert.strictEqual(
          Node._findSchema(schema, {}, path),
          schema.patternProperties['^foo[0-9]'].properties.fooChild.properties
            .fooChild2,
