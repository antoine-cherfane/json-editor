...
describe('util', () => {
  describe('jsonPath', () => {
    it('should stringify an array of paths', () => {
      ...
      assert.deepStrictEqual(
        stringifyPath(['foo', 2, 'bar_baz']),
        '.foo[2].bar_baz'
      )
      assert.deepStrictEqual(stringifyPath([2]), '[2]')
      assert.deepStrictEqual(
        stringifyPath(['foo', 'prop-with-hyphens']),
        '.foo["prop-with-hyphens"]'
      )
      assert.deepStrictEqual(
        stringifyPath(['foo', 'prop with spaces']),
    ...
    it('should parse a json path', () => {
      ...
      assert.deepStrictEqual(parsePath('.foo["prop with spaces"]'), [
        'foo',
        'prop with spaces'
      ])
      assert.deepStrictEqual(
      ...
      assert.deepStrictEqual(parsePath('.foo["prop with . dot"]'), [
        'foo',
        'prop with . dot'
      ])
  ...
  describe('isValidationErrorChanged', () => {
    const err1 = {
      ...
      schema: ['male', 'female'],
      parentSchema: { title: 'Gender', enum: ['male', 'female'] },
      data: null,
      type: 'validation'
    }
    const err2 = {
    ...
    it('same values for current and previoues errors should return false', () => {
      assert.strictEqual(
        isValidationErrorChanged([err1, err2, err3], [err2, err3, err1]),
        false
      )
    })
    ...
    it('different values for current and previoues errors should return true', () => {
      assert.strictEqual(
        isValidationErrorChanged([err1, err2], [err3, err1]),
        true
      )
    })
  })
  ...
  describe('makeFieldTooltip', () => {
    ...
    it('should make tooltips with only examples', () => {
      assert.strictEqual(
        makeFieldTooltip({ examples: ['foo', 'bar'] }),
        'Examples\n"foo"\n"bar"'
      )
    })
    ...
    it('should make tooltips with title and description', () => {
      assert.strictEqual(
        makeFieldTooltip({ title: 'foo', description: 'bar' }),
        'foo\nbar'
      )

      const longTitle = 'Lorem Ipsum Dolor'
      const longDescription =
        'Duis id elit non ante gravida vestibulum non nec est. ' +
    ...
    it('should handle empty fields', () => {
      assert.strictEqual(
        makeFieldTooltip({ title: '', description: 'bar' }),
        'bar'
      )
      assert.strictEqual(
        makeFieldTooltip({ title: 'foo', description: '' }),
        'foo'
      )
      assert.strictEqual(
        makeFieldTooltip({ description: 'bar', examples: [] }),
        'bar'
      )
      assert.strictEqual(
        makeFieldTooltip({ description: 'bar', examples: [''] }),
    ...
    it('should internationalize "Defaults" correctly', () => {
      assert.strictEqual(
        makeFieldTooltip({ default: 'foo' }, 'pt-BR'),
        'Revelia\n"foo"'
      )
    })
    ...
    it('should internationalize "Examples" correctly', () => {
      assert.strictEqual(
        makeFieldTooltip({ examples: ['foo'] }, 'pt-BR'),
        'Exemplos\n"foo"'
      )
    })
  })
  ...
  describe('sort', () => {
    ...
    it('should sort an array containing objects', () => {
      const array = [{ value: 4 }, { value: 1 }, { value: 10 }, { value: 2 }]
  ...
  describe('sortObjectKeys', () => {
    it('should sort the keys of an object', () => {
      ...
      assert.strictEqual(
        JSON.stringify(sortObjectKeys(object)),
        '{"a":"a","b":"b","c":"c"}'
      )
      assert.strictEqual(
        JSON.stringify(sortObjectKeys(object, 'asc')),
        '{"a":"a","b":"b","c":"c"}'
      )
      assert.strictEqual(
        JSON.stringify(sortObjectKeys(object, 'desc')),
        '{"c":"c","b":"b","a":"a"}'
      )
    })
  })
  ...
  it('should find a unique name', () => {
    ...
    assert.strictEqual(
      findUniqueName('b (copy)', ['a', 'b', 'b (copy)', 'b (copy 2)', 'c']),
      'b (copy 3)'
    )

    assert.strictEqual(
      findUniqueName('b (copy 2)', ['a', 'b', 'b (copy)', 'b (copy 2)', 'c']),
      'b (copy 3)'
    )
  })
  ...
  describe('uniqueMergeArrays', () => {
    it('should merge arrays with unique values', () => {
      ...
      const arr2 = ['c', 'd', 'f', 'g']
      assert.deepStrictEqual(uniqueMergeArrays(arr1, arr2), [
        'a',
        'b',
        'c',
        'd',
